// Description: This file contains the declaration of the inference class.
...
public:
    ...
    // Public functions
    std::vector<Object> run_inference(const std::string &image_path);
...
private:
    ...
    // Constructor
    InferencePipeline::InferencePipeline(const std::string &engine_file_path);
    // Destructor
    InferencePipeline::~InferencePipeline();

};

// Write a class CameraPipeline which will fetch images from camera and pass it to InferencePipeline
class CameraPipeline
{
public:
    // Public functions
    cv::Mat run_pipeline(const cv::VideoCapture);
    void set_cam_device(const std::string &camera);

private:
    // Private variables
    std::string camera;
    cv::VideoCapture cam_device;

    // Constructor
    CameraPipeline::CameraPipeline(const std::string &image_path);
    // Destructor
