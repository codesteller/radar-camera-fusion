...
void BirdViewModel::make_bev_matrix() {
    ...
    std::cout << "birdview_transform_matrix " << birdview_transform_matrix << std::endl;
}
...
void BirdViewModel::transformPoints(const std::vector<cv::Point2f> &normalized_points, std::vector<cv::Point2f> &normalized_dst_points) {
    ...
    perspectiveTransform(normalized_points, normalized_dst_points, this->birdview_transform_matrix);
