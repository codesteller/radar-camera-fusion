...
void BirdViewModel::make_bev_matrix() {
  std::lock_guard<std::mutex> guard(mtx);
  birdview_transform_matrix = cv::getPerspectiveTransform(
      four_image_points.to_vector(), four_points.to_vector());

  if (LOG_DEBUG_FLAG) {
    if (birdview_transform_matrix.empty()) {
      std::cout << "birdview_transform_matrix is empty" << std::endl;
    }

    std::cout << "birdview_transform_matrix " << birdview_transform_matrix
              << std::endl;
  }
}
...
void BirdViewModel::transformPoints(
    const std::vector<cv::Point2f> &normalized_points,
    std::vector<cv::Point2f> &normalized_dst_points) {
  std::lock_guard<std::mutex> guard(mtx);
  ...
  perspectiveTransform(normalized_points, normalized_dst_points,
                       this->birdview_transform_matrix);
}
...
cv::Mat BirdViewModel::getDangerZone(const cv::Size img_size,
  ...
  cv::Mat danger_mask(cv::Size(kBirdviewImgWidth, kBirdviewImgHeight), CV_8UC1,
                      cv::Scalar(0));
  ...
  cv::warpPerspective(danger_mask, transformed_danger_mask,
                      inv_birdview_transform_matrix, img_size);
